...
class SimplifiedChartPOC {
  ...
  /**
   * Get dev token from backend with correct tenantId
   * Using manager role to allow x-tenant-id header override
   */
  async getDevToken(): Promise<string | null> {
    try {
      // Request manager token (allows x-tenant-id override in auth middleware)
      const response = await fetch(`${this.backendUrl}/api/dev/token?tenantId=26&role=manager`);
      if (response.ok) {
  ...
  async setupPage(): Promise<Page> {
    ...
    await page.evaluate((token) => {
      ...
      // Set role to manager (allows x-tenant-id header override)
