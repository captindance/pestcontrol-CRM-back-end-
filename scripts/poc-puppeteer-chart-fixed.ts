...
class SimplifiedChartPOC {
  ...
  /**
   * Get dev token from backend with correct tenantId and userId
   * Using business_owner role with real userId from database
   */
  async getDevToken(): Promise<string | null> {
    try {
      // Request token with userId=35 (real user with business_owner access to client 26)
      const response = await fetch(`${this.backendUrl}/api/dev/token?userId=35&tenantId=26&role=business_owner`);
      if (response.ok) {
  ...
  async setupPage(): Promise<Page> {
    ...
    await page.evaluate((token) => {
      ...
      // Set tenant (client 26 - where reports exist)
      localStorage.setItem('selected_tenant_id', '26');
      
      // Set role to business_owner (matches user 35's role)
