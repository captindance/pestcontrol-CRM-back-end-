 ...
 */
export async function getUserClientIds(userId) {
    const userRoles = await prisma.userRole.findMany({
        where: { userId },
        select: { clientId: true }
    });
    // Filter out null clientIds (platform roles) and deduplicate
    const clientIds = userRoles.map(ur => ur.clientId).filter((id) => id !== null);
 ...
 */
export async function getUserPrimaryClientId(userId) {
 ...
 */
export async function getUserRoleForClient(userId, clientId) {
    ...
    if (userRole.role === 'manager' && !userRole.managerActive)
        return null;
    return userRole.role;
}
//# sourceMappingURL=clientAccess.js.map