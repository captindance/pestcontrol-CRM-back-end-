...
export function enqueueReport(reportId, clientId) {
...
function mockGenerateData(query) {
...
function randomValues(n, min, max) { return Array.from({ length: n }, () => Math.floor(Math.random() * (max - min) + min)); }
function monthValues(n) { return randomValues(n, 50, 200); }
function months(n) { const m = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']; return m.slice(0, n); }
function days(n) { return Array.from({ length: n }, (_, i) => `Day ${i + 1}`); }
