...
async function saveSettings(input) {
    ...
    const existing = await prisma.integrationSetting.findFirst({ where: { kind: 'email', provider: 'smtp' } });
...
function createTransport(config, opts) {
...
export function buildVerificationEmail(firstName, verificationLink) {
...
export function buildInvitationEmail(displayName, invitationLink) {
...
const signupEventLog = [];
const MAX_LOG_ENTRIES = 100;
export function logSignupEvent(event, email, details) {
    const entry = {
...
export function getSignupEventLog() {
