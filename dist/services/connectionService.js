...
function toSafe(conn, includeDetails = false) {
    const safe = {
        id: conn.id,
...
async function ensureConnectionOwned(id, clientId) {
    const conn = await prisma.databaseConnection.findUnique({ where: { id } });
    if (!conn || conn.clientId !== clientId)
        return null;
    return conn;
}
export async function listConnections(clientId, includeDetails = false) {
...
export async function createConnection(clientId, input) {
    const encData = {
        engine: input.engine,
        host: input.host,
        port: input.port,
...
export async function updateConnection(clientId, id, input) {
...
export async function deleteConnection(clientId, id) {
    const existing = await ensureConnectionOwned(id, clientId);
    if (!existing)
        return false;
...
export async function getDecryptedConnection(clientId, id) {
...
export async function testConnection(clientId, id, overridePassword, unsavedData) {
    let conn;
    if (unsavedData) {
        ...
        if (!id)
            return { ok: false, message: 'Connection ID required' };
        conn = await getDecryptedConnection(clientId, id);
        if (!conn)
            return { ok: false, message: 'Connection not found for tenant' };
...
export async function connectionInUse(id) {
