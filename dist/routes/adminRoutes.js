...
router.post('/users', async (req, res) => {
    if (!requirePlatformAdmin(req, res))
        return;
    const { email, role, clientId, firstName } = req.body || {};
    const normalizedEmail = email?.trim().toLowerCase();
    if (!normalizedEmail || !role)
        return res.status(400).json({ error: 'Missing email or role' });
    try {
        if ((role === 'business_owner' || role === 'delegate') && !clientId)
            return res.status(400).json({ error: 'clientId required for business_owner/delegate' });
        const existingUser = await prisma.user.findUnique({ where: { email: normalizedEmail } });
        if (existingUser) {
        ...
        if (role === 'platform_admin') {
            ...
            if (clientIdNum && (role === 'business_owner' || role === 'delegate')) {
                await prisma.userRole.create({ data: { userId: created.id, clientId: clientIdNum, role: role } });
            }
        }
        const result = { id: created.id, email: created.email, role: role, emailVerified: created.emailVerified };
        if (clientIdNum) {
...
router.get('/managers/:userId/assignments', async (req, res) => {
    if (!requirePlatformAdmin(req, res))
        return;
...
router.post('/managers/:userId/assignments', async (req, res) => {
    if (!requirePlatformAdmin(req, res))
        return;
    const userId = parseInt(req.params.userId);
    const clientIdParam = req.body?.clientId;
    if (!clientIdParam)
    ...
    try {
        ...
        if (!user.emailVerified)
            return res.status(400).json({ error: 'Manager must verify email before assignments' });
...
router.patch('/managers/:userId/assignments/:clientId', async (req, res) => {
    if (!requirePlatformAdmin(req, res))
        return;
    const userId = parseInt(req.params.userId);
    ...
    if (typeof active !== 'boolean')
        return res.status(400).json({ error: 'Missing active boolean' });
    try {
        const user = await prisma.user.findUnique({ where: { id: userId } });
        if (!user)
            return res.status(404).json({ error: 'User not found' });
...
router.post('/managers/:userId/resend-verification', async (req, res) => {
    if (!requirePlatformAdmin(req, res))
        return;
    const userId = parseInt(req.params.userId);
    try {
...
router.post('/email-test', async (req, res) => {
    ...
    try {
        ...
        const result = await sendMail(testEmail, 'Test Email from PestControl CRM', 'This is a test email to verify your SMTP configuration is working correctly.', '<p>This is a test email to verify your SMTP configuration is working correctly.</p><p>If you received this, email sending is operational!</p>');
        if (result.sent) {
            res.json({
                success: true,
