...
function ensureCanEdit(req, res) {
    if (req.user && req.user.roles?.some(role => editableRoles.has(role)))
        return true;
...
async function ensureClientAccess(req, res, clientId) {
    if (!req.user) {
...
router.get('/', async (req, res) => {
    ...
    if (!tenantId)
        return;
    if (!(await ensureClientAccess(req, res, tenantId)))
        return;
...
router.post('/test', async (req, res) => {
    ...
    try {
        ...
        if (!result.ok)
            return res.status(400).json({ ok: false, message: result.message || 'Test failed' });
        res.json({ ok: true });
    }
    catch (e) {
