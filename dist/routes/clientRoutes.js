...
function authorizeOwnerOrAdmin(req, res, next) {
    ...
    if (roles.includes('platform_admin'))
        return next();
    if ((roles.includes('business_owner') || roles.includes('delegate')) && req.tenantId === targetClientId)
        return next();
...
router.get('/all', async (req, res) => {
    try {
        ...
        const clientsWithRole = await Promise.all(clients.map(async (client) => {
...
router.get('/:id/users', authorizeOwnerOrAdmin, async (req, res) => {
...
router.post('/:id/users', authorizeOwnerOrAdmin, async (req, res) => {
    const clientId = parseInt(req.params.id);
    ...
    `;
        const mailResult = await sendMail(user.email, 'Invitation to Join PestControl CRM', undefined, emailHtml);
        res.status(201).json({
            id: user.id,
            email: user.email,
            role: role,
...
router.get('/:id/managers', authorizeOwnerOrAdmin, async (req, res) => {
    const clientId = parseInt(req.params.id);
    try {
...
router.post('/:id/delegates', authorizeOwnerOrAdmin, async (req, res) => {
    const clientId = parseInt(req.params.id);
...
router.get('/:id/users/:userId/permissions', authorizeOwnerOrAdmin, async (req, res) => {
    const clientId = parseInt(req.params.id);
...
router.patch('/:id/users/:userId/permissions', authorizeOwnerOrAdmin, async (req, res) => {
    const clientId = parseInt(req.params.id);
    const targetUserId = parseInt(req.params.userId);
    // Check if current user has permission to manage users
    const currentUserId = parseInt(req.user.userId);
    ...
    const { canViewReports, canCreateReports, canEditReports, canDeleteReports, canManageConnections, canInviteUsers, canManageUsers: canManageUsersUpdate } = req.body;
    try {
