...
import { sendMail, buildVerificationEmail, logSignupEvent } from '../services/emailService.js';
import { getJWTConfig } from '../config/jwt.config.js';
const router = Router();
// Check if any users exist (public endpoint for signup form)
router.get('/has-users', async (_req, res) => {
    try {
...
router.post('/signup', async (req, res) => {
    const { email, firstName, lastName, clientId, companyName, password, token } = (req.body || {});
...
router.post('/verify-email', async (req, res) => {
    ...
    try {
        ...
        await Promise.all([
            ...
            prisma.invitation.updateMany({
                where: {
                    ...
                    invitationType: 'account_creation',
                    status: 'pending'
                },
                data: { status: 'accepted', acceptedAt: new Date(), updatedAt: new Date() }
            })
        ...
        if (global.notifyManagerUpdate) {
            global.notifyManagerUpdate();
        }
