...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (!validConn)
            return res.status(400).json({ error: 'Invalid connectionId for this tenant' });
...
router.post('/:id/run', async (req, res) => {
    ...
    if (report.status === 'running')
        return res.status(409).json({ error: 'Report already running' });
...
router.post('/:id/execute-query', async (req, res) => {
    if (!req.tenantId) {
        return res.status(400).json({ error: 'Client context required; provide x-tenant-id header' });
    }
    const tenantId = parseInt(req.tenantId);
...
router.put('/:id/chart-config', async (req, res) => {
    if (!req.tenantId) {
        return res.status(400).json({ error: 'Client context required; provide x-tenant-id header' });
    }
    const tenantId = parseInt(req.tenantId);
